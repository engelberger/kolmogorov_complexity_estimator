...
def test_simulation_pipeline_raw_sums_to_one_and_rank(n):
    """
    End-to-end pipeline for small n with raw enumeration, verifying distribution sums to 1 and K ranking.
    """
    ...
    for tm_table in enumerator:
        if has_no_halt_transition(tm_table):
            ...
            status, output, filter_name = tm.run(max_steps=10, runtime_filters=runtime_filters)
