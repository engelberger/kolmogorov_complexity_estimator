...
from kolmogorov_complexity_estimator.reduction_filters import check_for_escapee, check_for_cycle_two
...
def test_escapee_filter_triggers():
    ...
    table = {(1, '0'): (1, '0', 1)}
    tm = TuringMachine(1, table)
    status, output, filter_name = tm.run(max_steps=10, runtime_filters=[check_for_escapee])
...
def test_cycle_two_filter_triggers():
    ...
    status, output, filter_name = tm.run(max_steps=10, runtime_filters=[check_for_cycle_two])
    assert status == 'filtered'
    assert output is None
