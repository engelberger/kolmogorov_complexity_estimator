...
def has_no_halt_transition(
    ...
    :param tm_table: Mapping from (state, read_symbol) to (next_state,
...
def check_for_escapee(tm) -> bool:
    """
    Detect escapee: TM moving over fresh blank cells consecutively more than its
    number of states. Returns True to signal the machine is escaping.
    """
    ...
    if not hasattr(tm, "_escapee_seen_positions"):
        ...
        return False
    # Determine if current head pos is a new blank cell
    # (use defaultdict to detect blank)
...
def check_for_cycle_two(tm) -> bool:
    """
    Detect 2-step cycles: if at steps s and s+2 the (state, head_position, tape)
    are identical. Returns True to signal the machine is caught in a period-two loop.
    """
...
def apply_completion_rules(
    output_counts: Counter, non_halting_count: int, M_red: int, n_states: int
) -> Tuple[Counter, int, int, int]:
    """
    Complete output and non-halting counts after running M_red machines in reduced
    enumeration.
